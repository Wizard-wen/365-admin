...
export default {
    ...
    data(){
        return {
            typeList: [
                {type: 'sale_amount', sortTitle: '订单流水排行',name: '流水',isActive: true},
                {type: 'sale_service_amount', sortTitle: '营收（销售额）排行',name: '销售额',isActive: false},
                {type: 'order_transform_rate', sortTitle: '订单转化率排行排行',name: '转化率',isActive: false},
            ],
        }
    },
    computed: {
        sortList(){
            ...
            } else if(currentKey == 'order_transform_rate'){
                sortList = sortList.map(item =>{
                    return {
                        ...item,
