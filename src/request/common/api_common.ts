...
export interface api_common {
    ...
    // 获取某个部门的全部员工
    getDepartmentManagerSelection:{(department_id:number):Promise<any>},
    // 获取员工树形结构
    getManagerTree:{():Promise<any>},
    // 获取全部门店列表
    getStoreSelection:{():Promise<any>},
    // 获取某个门店员工列表
    getStoreManagerSelection:{(store_id:number):Promise<any>},
    // 获取当前未绑定门店员工
    getNotInStoreManagerSelection:{(store_id:number):Promise<any>},
    // 上传图片
